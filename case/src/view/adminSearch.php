...
<body>
<h1>
    <button style="font-size: 50px" class="btn btn-info">Kết quả tìm kiếm</button>
</h1>
<br>
<div align="center">
    ...
    <a class="btn btn-success" href="index.php?page=adminCancel">Quay lại</a>
    <a class="btn btn-danger" href="index.php?page=menu&id=4">Đăng xuất</a>
</div>
<br>
<?php if(!$results): ?>
    <h3 style="text-align: center;color: darkred">Không có sản phẩm bạn vừa tìm!</h3>
<?php else:?>
<h4 style="text-align: center;color: darkred">Có <?= count($results)?> sản phẩm phù hợp với kết quả bạn vừa tìm kiếm:</h4>
<table align="center" class="table" style="width: 90%;border-radius: 15px">
    ...
    foreach ($results as $key => $value): ?>
        <tr>
            ...
            <td><?php echo number_format($value->getPrice(), 0, ",", "."); ?> VND</td>
            <td style="text-align: center"><img style="width: 150px;height: 200px" src="image/<?php echo $value->getImage(); ?>.jpg" alt="Hình ảnh sản phẩm"></td>
            <td style="text-align: center"><?php echo $value->getBelongToMenu();?></td>
            <td style="text-align: center">
                <a class="btn btn-primary" href="index.php?page=adminUpdate&id=<?php echo $value->getId(); ?>">Cập nhật</a>
                <a class="btn btn-warning" onclick="return confirm('Đây là dữ liệu gốc! Bạn thật sự muốn xóa nó?')"
                   href="index.php?page=adminDelete&id=<?php echo $value->getId(); ?>">Xóa</a>
            </td>
        </tr>
    <?php endforeach; ?>
    <?php endif;?>
